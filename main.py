...
from generate_txt import Text_Generator
...
class Chat_Window(tk.Toplevel):
    def __init__(self, parent):
        ...
        self.layout.columnconfigure(0, weight=3)
        self.layout.columnconfigure(1, weight=1)
        self.layout.rowconfigure(0, weight=2)
        self.layout.rowconfigure(1, weight=1)
        self.layout.rowconfigure(2, weight=10)

        self.entered_word = tk.StringVar()
        self.word_entry = ttk.Entry(self.layout, textvariable=self.entered_word)
        self.word_entry.grid_configure(column=0, row=0, sticky="news")

        self.submit_word = ttk.Button(self.layout, text="SUBMIT", command=self.submit_pressed)
        self.submit_word.grid_configure(column=1, row=0, sticky="news", rowspan=2)

        self.output = tk.StringVar()
        self.output_label = ttk.Label(self.layout, textvariable=self.output)
        self.output_label.grid_configure(column=0, row=2, columnspan=2, sticky="news")
        self.output_label.bind('<Configure>', lambda e: self.output_label.config(wraplength=self.output_label.winfo_width()))
        self.output.set("OUTPUT")

        self.word_limit = tk.IntVar(value=500)

        self.slider = tk.Scale(self.layout, from_=0, to=1000, variable=self.word_limit, resolution=1, orient="horizontal")
        self.slider.grid_configure(column=0, row=1, sticky="nsew")
        
    def submit_pressed(self):
        output_text = Text_Generator.from_one_word(self.parent.rel_dict, self.entered_word.get().upper(), self.word_limit.get())
        self.output.set(output_text)
